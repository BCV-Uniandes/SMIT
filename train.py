...
class Train(Solver):
    ...
    def run(self):
        ...
        if start == 0:
            name = os.path.join(
                self.config.sample_path, '{}_{}_fake.jpg'.format(
                    str(0).zfill(4),
                    str(0).zfill(len(str(last_model_step)))))
            if not self.config.Deterministic:
            ...
            self.save_fake_output(
                fixed_x, name, label=fixed_label, training=True)

        self.PRINT("Current time: " + TimeNow())

        # Tensorboard log path
        if self.config.use_tensorboard:
            self.PRINT("Tensorboard Log path: " + self.config.log_path)
        ...
        for e in range(start, self.config.num_epochs):
            ...
            self.G.train()
            self.LOSS = {}
            desc_bar = '[Iter: %d] Epoch: %d/%d' % (_iter, e,
            ...
            for i, (real_x, real_c, files) in progress_bar:
                ...
                #===================================DEBUG====================================#
                # PRINT log info
                if (i + 1) % self.config.log_step == 0 or (
                        i + 1) == last_model_step or i + e == 0:
                    progress_bar.set_postfix(**self.loss)
                    if (i + 1) == last_model_step: progress_bar.set_postfix('')
                    if self.config.use_tensorboard:
                        for tag, value in self.loss.items():
                            self.logger.scalar_summary(
                                tag, value, e * last_model_step + i + 1)
                # Save current fake
                if ((i + 1) % self.config.sample_step == 0 or
                    (i + 1) == last_model_step
                        or i + e == 0) and self.config.image_size <= 128:
            ...
            if e % self.config.save_epoch == 0:
                ...
                log = '--> %s | Elapsed [Iter: %d] (%d/%d) : %s | %s\nTrain' % (
                    TimeNow(), _iter, e, self.config.num_epochs, elapsed, Log)
