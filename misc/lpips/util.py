...
def datetime_str():
    now = datetime.now()
    return '%04d-%02d-%02d-%02d-%02d-%02d' % (now.year, now.month, now.day,
...
def bootstrap(in_vec, num_samples=100, bootfunc=np.mean):
    ...
    return stats.bootstrap(
        np.array(in_vec), bootnum=num_samples, bootfunc=bootfunc)
...
def cos_sim_blob(in0, in1):
    ...
    return np.mean(
        np.mean(np.sum(in0_norm * in1_norm, axis=1), axis=1), axis=1)
...
def cos_sim(in0, in1):
    ...
    return torch.mean(
        torch.mean(
            torch.sum(in0_norm * in1_norm, dim=1).view(N, 1, X, Y),
            dim=2).view(N, 1, 1, Y),
        dim=3).view(N)
