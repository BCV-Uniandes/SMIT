  ...
  def save_fake_output(self, real_x, save_path, label=None, gif=False, output=False, many_faces=False, training=False, Style=0, fixed_style=None, TIME=False):
    ...
    with opt:
      ...
      for idx, real_x0 in enumerate(real_x):
        ...
        if not Deterministic: Output.extend(self._SAVE_IMAGE(_save_path, fake_image_list, attn_list=fake_attn_list, im_size=self.config.image_size, gif=gif, mode='style_'+chr(65+idx), style=Style))
        if idx==self.config.iter_style or (training and idx==self.config.style_train_debug-1) or Deterministic: break
    self.G.train()
    self.D.train()
