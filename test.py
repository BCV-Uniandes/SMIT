...
class Test(Solver):
  ...
  def save_multimodal_output(self, real_x, label, save_path, interpolation=False, **kwargs):
    self.G.eval()  
    self.D.eval()
    n_rep = 4
    opt = torch.no_grad() if get_torch_version()>0.3 else open('/tmp/_null.txt', 'w')
    ...
    with opt:
      ...
      for idx, real_x0 in enumerate(real_x):
        ...
        _save_path = os.path.join(save_path.replace('.jpg', ''), '{}_{}.jpg'.format(_name, str(idx).zfill(4)))
        ...
        fake_image_list = [color_frame(single_source(real_x0), thick=5, color='green', first=True)]
        if 'Attention' in self.config.GAN_options:
          fake_attn_list = [color_frame(single_source(real_x0), thick=5, color='green', first=True)]
        ...
        for n_label, _target_c in enumerate(target_c_list):
          ...
          if not interpolation:
            ...
            z0 = to_data(self.G.random_style(1), cpu=True).numpy()[0]; z1 = to_data(self.G.random_style(1), cpu=True).numpy()[0]
            style_ = self.G.random_style(n_rep)
            style_[:] = torch.FloatTensor(np.array([slerp(sz, z0, z1) for sz in np.linspace(0,1,n_rep)]))          
        ...
        self._SAVE_IMAGE(_save_path, fake_image_list, im_size=self.config.image_size, mode='style_'+chr(65+idx), no_labels=True)
        if 'Attention' in self.config.GAN_options:
          self._SAVE_IMAGE(_save_path.replace(_name, _name+'_attn'), fake_attn_list, im_size=self.config.image_size, mode='style_'+chr(65+idx), no_labels=True)
    self.G.train()
    self.D.train()
  ...
  def __call__(self, dataset='', load=False):
    ...
    for i, (real_x, org_c, files) in enumerate(data_loader):
      ...
      if self.config.dataset_fake in ['Image2Edges', 'Yosemite']:
        output = self.save_multimodal_output(real_x, 1-org_c, name, interpolation=True)
